...
SRC_URI="
	x86?	( http://www.wxtoimg.com/beta/wxtoimg-linux-i386-${PVR}-beta.tar.gz -> ${PF}-i386-beta.tar.gz )
	amd64?	( http://www.wxtoimg.com/beta/wxtoimg-linux-amd64-${PVR}-beta.tar.gz -> ${PF}-amd64-beta.tar.gz  )
	arm?	( http://www.wxtoimg.com/beta/wxtoimg-linux-armhf-${PVR}-beta.tar.gz -> ${PF}-armhf-beta.tar.gz  )
"
...
src_unpack() {
	if use x86; then
		unpack ${PF}-x86-beta.tar.gz
	fi

	if use amd64; then
		unpack ${PF}-amd64-beta.tar.gz
	fi
	if use arm; then
		unpack ${PF}-armhf-beta.tar.gz
	fi
